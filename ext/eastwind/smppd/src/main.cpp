...
namespace local {
	...
	typedef ewgate::tcp_service<smpp::malloc_allocator, log_t> ewgate_service;

	class smppservice: public smpp_service {

		public:
			smppservice(const ba::ip::tcp::endpoint & endpoint
	...
	class ewgateservice: public ewgate_service {

		public:
			ewgateservice(const ba::ip::tcp::endpoint & endpoint
					, smpp::malloc_allocator & a
					, log_t l)
				: ewgate_service(endpoint, a, std::move(l))
			{
				msg_id = 0;
			}

			virtual ~ewgateservice()
			{
			}

		protected:
			using ewgate_service::L;
			...
			void on_heartbeat(bin::sz_t channel_id, const ewgate::heartbeat & msg) {
				ltrace(L) << "channel #" << channel_id << " got: " << msg;
			}
	};
...
int main(int argc, char ** argv)
{
	...
	try {
		...
		local::smppservice smppd(endpoint, allocator, vision::log::channel("smppd"));
		local::ewgateservice ewgated(endpoint, allocator, vision::log::channel("ewgated"));
